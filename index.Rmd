...
tweets$created <- as_datetime(tweets$created)
...
top_rt <- data.frame(tweets %>%
    filter(!is_retweet) %>%
    filter(!screen_name %in% c('meeting_goer')) %>%
...
Veamos quienes fueron los m?s activos de entre los `r length(unique(ICOPFIRES_tweets$screen_name))` usuarios que tuitearon sobre el congreso utilizando la etiqueta oficial. 
