...
tweets %>%
  ggplot(aes(created, fill = is_retweet)) +
...
tweets$bin <- cut(as.numeric(tweets$created), breaks = 70)
...
names(mentions) <- original_tweets$screenName
...
V(mention_graph)$group <- ifelse(V(mention_graph)$name %in% original_tweets$screenName, "HashtagUser", "NonHashtagUser")
